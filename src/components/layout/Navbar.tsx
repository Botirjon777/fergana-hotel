  ...
  return (
    ...
    >
      ...
      <div className="flex md:hidden items-center gap-4">
        <LanguageSelector isScrolled={scrolled} />
        <div className="flex flex-col gap-[5px] cursor-pointer p-2" onClick={openSidebar}>
          <span className={`block w-6 h-[1.5px] transition-colors duration-300 ${scrolled ? 'bg-gold' : 'bg-cream'}`}></span>
          <span className={`block w-6 h-[1.5px] transition-colors duration-300 ${scrolled ? 'bg-gold' : 'bg-cream'}`}></span>
          <span className={`block w-6 h-[1.5px] transition-colors duration-300 ${scrolled ? 'bg-gold' : 'bg-cream'}`}></span>
        </div>
      </div>
