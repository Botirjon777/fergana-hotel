...
import { FiCalendar, FiUsers, FiChevronLeft, FiChevronRight, FiMinus, FiPlus, FiArrowRight } from "react-icons/fi";
...
export function AvailabilityModal() {
  ...
  const renderCalendar = (monthOffset: number) => {
    ...
    for (let i = 0; i < firstDay; i++) days.push(<div key={`empty-${monthOffset}-${i}`} />);
    ...
    for (let d = 1; d <= totalDays; d++) {
      ...
      let inRange = false;
      if (currentCheckIn && currentCheckOut) inRange = dateString > currentCheckIn && dateString < currentCheckOut;
      ...
      days.push(
        <div
          ...
          className={`h-11 md:h-12 flex flex-col items-center justify-center cursor-pointer text-[13px] transition-all duration-200 border border-transparent ${
            isStart || isEnd ? "bg-gold text-white font-bold" : 
            inRange ? "bg-gold/20 text-white" : 
            disabled ? "opacity-20 cursor-not-allowed" : "hover:bg-white/10 text-white/80 hover:border-gold/30"
          }`}
        >
          <span>{d}</span>
          {!disabled && lowestPrices[dateString] && (
             <span className={`text-[8px] md:text-[9px] mt-0.5 ${isStart || isEnd ? "text-white/80" : "text-gold"}`}>
  ...
  return (
    <Modal isOpen={isOpen} onClose={closePopup} type="full">
      <div className="bg-[#1a1108] w-full h-full md:h-auto md:max-w-[800px] relative overflow-hidden border-none md:border md:border-gold/20 md:shadow-2xl flex flex-col">
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="flex flex-col">
              ...
              <div className="flex gap-2 mt-1">
                <div className={`h-1 w-8 rounded-full transition-colors ${step === 1 ? "bg-gold" : "bg-gold/20"}`}></div>
                <div className={`h-1 w-8 rounded-full transition-colors ${step === 2 ? "bg-gold" : "bg-gold/20"}`}></div>
              </div>
            </div>
          </div>
          <button onClick={closePopup} className="text-white/40 hover:text-white text-3xl transition-colors p-2">
            &times;
          </button>
        </div>
          ...
          >
            ...
            <div className="w-full shrink-0 p-4 md:p-8 flex flex-col gap-6 overflow-y-auto custom-scrollbar">
              <div className="flex justify-between items-center px-2">
                 <button onClick={() => setViewDate(new Date(viewDate.getFullYear(), viewDate.getMonth() - 1, 1))} className="text-gold p-2 hover:bg-gold/10 rounded-full transition-colors"><FiChevronLeft className="text-xl"/></button>
                 ...
                 <button onClick={() => setViewDate(new Date(viewDate.getFullYear(), viewDate.getMonth() + 1, 1))} className="text-gold p-2 hover:bg-gold/10 rounded-full transition-colors"><FiChevronRight className="text-xl"/></button>
              </div>
              ...
              <div className="flex flex-col md:flex-row gap-8">
                ...
                <div className="flex-1">
                  <div className="grid grid-cols-7 gap-1 text-[10px] md:text-[11px] text-gold/60 text-center uppercase tracking-widest mb-4">
                    {weekDays.map((w: string) => <div key={w} className="py-1">{w}</div>)}
                  </div>
            ...
            <div className="w-full shrink-0 p-6 md:p-12 flex flex-col gap-10 overflow-y-auto custom-scrollbar">
               <div className="space-y-10 max-w-[500px] mx-auto w-full">
                  ...
                  <div className="flex justify-between items-center p-6 bg-white/5 rounded-xl border border-white/10 group hover:border-gold/30 transition-colors">
                     <div>
                       <h3 className="text-white font-medium text-xl md:text-2xl">{tg("adults")}</h3>
                     ...
                     <div className="flex items-center gap-6">
                       ...
                       <span className="text-white font-bold text-2xl w-8 text-center">{adults}</span>
                       <button 
                  ...
                  <div className="flex justify-between items-center p-6 bg-white/5 rounded-xl border border-white/10 group hover:border-gold/30 transition-colors">
                     <div>
                       <h3 className="text-white font-medium text-xl md:text-2xl">{tg("children")}</h3>
                     ...
                     <div className="flex items-center gap-6">
                       ...
                       <span className="text-white font-bold text-2xl w-8 text-center">{childrenAges.length}</span>
                       <button 
                  ...
                  {childrenAges.length > 0 && (
                    <div className="flex flex-col gap-6 pt-6 border-t border-gold/10">
                      {childrenAges.map((age, i) => (
                        <div key={i} className="flex justify-between items-center animate-modal-in">
                           <span className="text-white/70 text-base">{tg("childAge", { index: i + 1 })}</span>
                           <CustomAgeSelect value={age} onChange={(newAge) => {
        ...
        <div className="p-6 md:p-8 border-t border-gold/10 bg-[#1f160d] shrink-0">
          <div className="max-w-[500px] mx-auto w-full">
            {step === 1 ? (
              ...
              >
                {t("next")} <FiArrowRight className="text-lg group-hover:translate-x-2 transition-transform" />
              </button>
            ) : (
              <button 
