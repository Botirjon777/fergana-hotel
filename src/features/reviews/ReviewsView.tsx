...
export default function ReviewsView() {
  ...
  const { data: reviewsData, isLoading: isLoadingReviews } = useDetailedReviews(
    locale,
    20,
  );
  ...
  return (
    <main className="min-h-screen bg-[#1a1108] text-white">
      ...
      <section className="py-10 px-4 md:px-6 max-w-[1200px] mx-auto min-h-[70vh]">
              ...
              {summary && (
                <div className="bg-white/5 border border-gold/20 p-6 md:p-8 rounded-lg sticky top-32">
                  <div className="flex items-end gap-3 mb-2">
                    <span className="text-5xl font-cormorant text-gold">
                      {summary.rate.rate}
                    </span>
                    <span className="text-white/50 mb-1">/ 10</span>
                  </div>
                  <div className="text-white/90 text-xl capitalize mb-1">
            ...
            <div className="lg:col-span-2 space-y-6">
              {reviewsData?.reviews.map((review) => (
                <div
                  ...
                  className="bg-white/5 border border-white/10 p-6 rounded-lg transition-colors hover:border-gold/30"
                >
