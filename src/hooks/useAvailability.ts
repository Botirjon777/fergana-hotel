    ...
    const fetchAvailability = async () => {
      ...
      try {
        ...
        const [rulesRes, availRes] = await Promise.all([
          fetch(`https://uz-ibe.hopenapi.com/ApiWebDistribution/AvailabilityCalendar/hotel_booking_rules?end_date=${endDate}&hotel=506786&shared=false&start_date=${startDate}`),
          fetch(`https://uz-ibe.hopenapi.com/ApiWebDistribution/AvailabilityCalendar/room_type_availability_2?aggregate_dates=true&currency=UZS&end_date=${endDate}&hotel=506786&max_nights=21&start_date=${startDate}&shared=false&point_of_sale.referrer_code=GOOG`)
        ]);
